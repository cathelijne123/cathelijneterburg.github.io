---
layout: default
title: Papers
permalink: /papers/
---

# Papers

Select a paper to read an accessible explanation.

<div class="paper-list">

<a class="paper-card" href="{{ '/papers/paper-1/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-1-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 1">
  <div class="paper-card-text">
    <h2>Paper 1 title</h2>
    <p>Short one-sentence description of Paper 1.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-2/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-2-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 2">
  <div class="paper-card-text">
    <h2>Paper 2 title</h2>
    <p>Short one-sentence description of Paper 2.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-3/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-3-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 3">
  <div class="paper-card-text">
    <h2>Paper 3 title</h2>
    <p>Short one-sentence description of Paper 3.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-4/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-4-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 4">
  <div class="paper-card-text">
    <h2>Paper 4 title</h2>
    <p>Short one-sentence description of Paper 4.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-5/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-5-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 5">
  <div class="paper-card-text">
    <h2>Paper 5 title</h2>
    <p>Short one-sentence description of Paper 5.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-6/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-6-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 6">
  <div class="paper-card-text">
    <h2>Paper 6 title</h2>
    <p>Short one-sentence description of Paper 6.</p>
  </div>
</a>

<a class="paper-card" href="{{ '/papers/paper-7/' | relative_url }}">
  <img
    src="{{ '/assets/images/papers/paper-7-preview.svg' | relative_url }}"
    alt="Preview figure for Paper 7">
  <div class="paper-card-text">
    <h2>Paper 7 title</h2>
    <p>Short one-sentence description of Paper 7.</p>
  </div>
</a>

</div>
